...
const Login = async (req, res) => {
    try {
        ...
        res.cookie("token", token, {
            httpOnly: true,
            sameSite: "lax", // Spell check: sameSite (aapka samsite tha)
            maxAge: 1 * 60 * 60 * 1000,
            secure: false
        });
        ...
        // res.status(200).send({
        //     message: "login successfully",
        //     token, // Token direct bhejein
        //     user: {
        //         id: user._id,
        //         name: user.name,
        //         email: user.email,
        //         role: user.role // YEH ZAROORI HAI
        //     }
        // });
        ...
        res.status(200).send({
            message: "login successfully",
            token: token, // Token bahar hona chahiye
            user: {
                role: user.role, // Role lazmi bhejein
                email: user.email,
                name: user.name
            }
        });
